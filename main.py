...
class ShoppingCartApp:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Frosty Flavours Shopping Cart App")
...
#  (tk as tkinter use for simple graphical interferance), (frame as container) ,(def is use for function or method)
    ...
    def show_main_menu(self):
        ...
        tk.Label(frame, text="˚☽˚｡⋆｡❅*WELCOME TO FROSTY FLAVOURS!˚☽˚｡⋆｡❅*", font=("Helvetica", 16)).pack(pady=10)
        tk.Button(frame, text="Register", command=self.show_register, width=20).pack(pady=5)
        tk.Button(frame, text="Login", command=self.show_login, width=20).pack(pady=5)
...
#registration
    ...
    def show_register(self):
        ...
        tk.Label(frame, text="Register", font=("Helvetica", 14)).pack(pady=10)
...
#login
    ...
    def show_login(self):
        ...
        tk.Label(frame, text="Login", font=("Helvetica", 14)).pack(pady=10)
...
#products
    ...
    def view_products(self, user):
        ...
        product_list_frame = tk.Frame(frame)
        product_list_frame.pack()

        scrollbar = tk.Scrollbar(product_list_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        product_listbox = tk.Listbox(product_list_frame, yscrollcommand=scrollbar.set, width=50)
        for product in self.products.values():
            product_listbox.insert(tk.END, str(product))
...
#cart
    ...
    def view_cart(self, user):
        ...
        if not user.cart.items:
            ...
            cart_list_frame = tk.Frame(frame)
            cart_list_frame.pack()

            scrollbar = tk.Scrollbar(cart_list_frame)
            scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

            cart_listbox = tk.Listbox(cart_list_frame, yscrollcommand=scrollbar.set, width=50)
            for item in user.cart.items.values():
                product = item['product']
                quantity = item['quantity']
                cart_listbox.insert(tk.END, f"{product.name} (x{quantity}): ${product.price * quantity}")
            cart_listbox.pack(side=tk.LEFT, fill=tk.BOTH)
